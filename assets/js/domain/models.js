...
/**
 * @returns {import('../core/types.js').Scene}
 */
...
/**
 * @returns {import('../core/types.js').Chapter}
 */
...
/**
 * @returns {import('../core/types.js').Character}
 */
...
/**
 * @returns {import('../core/types.js').BookEvent}
 */
...
/**
 * @returns {import('../core/types.js').NoteItem}
 */
 ...
 * @returns {import('../core/types.js').ExtraBlock}
 */
 ...
 * @returns {import('../core/types.js').WorldRule}
 */
 ...
 * @returns {import('../core/types.js').Act}
 */
...
/**
 * @returns {import('../core/types.js').Highlight}
 */
 ...
 * @returns {import('../core/types.js').EditorComment}
 */
...
/**
 * @returns {import('../core/types.js').Snapshot}
 */
...
/**
 * @returns {import('../core/types.js').KanbanTask}
 */
...
/**
 * @returns {import('../core/types.js').KanbanColumn}
 */
export function createKanbanColumn(overrides = {}) {
  return {
    id: uuid(),
    title: 'Columna',
    order: 0,
...
/**
 * Tablero con columnas Por hacer / Haciendo / Terminadas.
 * @returns {import('../core/types.js').KanbanBoard}
 */
export function createKanbanBoard(overrides = {}) {
  return {
    id: uuid(),
    name: 'Nuevo tablero',
    createdAt: new Date().toISOString(),
    columns: [
      createKanbanColumn({ title: 'Por hacer', order: 0 }),
      createKanbanColumn({ title: 'Haciendo', order: 1 }),
      createKanbanColumn({ title: 'Terminadas', order: 2 }),
    ],
    ...overrides,
  };
}
 ...
 * @returns {import('../core/types.js').Book}
 */
...
/**
 * @returns {import('../core/types.js').AuthorProfile}
 */
...
/**
 * @returns {import('../core/types.js').Workspace}
 */
 ...
 * @returns {import('../core/types.js').KanbanTask}
 */
function normalizeKanbanTask(raw) {
  const t = typeof raw === 'object' && raw !== null ? /** @type {Record<string, unknown>} */ (raw) : {};
  return createKanbanTask({
    id: typeof t.id === 'string' ? t.id : uuid(),
    title: typeof t.title === 'string' ? t.title : 'Tarea',
    description: typeof t.description === 'string' ? t.description : '',
    startDate: typeof t.startDate === 'string' ? t.startDate : '',
    dueDate: typeof t.dueDate === 'string' ? t.dueDate : '',
    backgroundColor: typeof t.backgroundColor === 'string' ? t.backgroundColor : '',
  });
}
 ...
 * @returns {import('../core/types.js').KanbanColumn}
 */
function normalizeKanbanColumn(raw) {
  const c = typeof raw === 'object' && raw !== null ? /** @type {Record<string, unknown>} */ (raw) : {};
  const tasks = Array.isArray(c.tasks) ? c.tasks.map(normalizeKanbanTask) : [];
  return createKanbanColumn({
    id: typeof c.id === 'string' ? c.id : uuid(),
    title: typeof c.title === 'string' ? c.title : 'Columna',
    order: typeof c.order === 'number' && !Number.isNaN(c.order) ? c.order : 0,
    tasks,
  });
}
 ...
 * @returns {import('../core/types.js').KanbanBoard}
 */
function normalizeKanbanBoard(raw) {
  const kb = typeof raw === 'object' && raw !== null ? /** @type {Record<string, unknown>} */ (raw) : {};
  let columns = Array.isArray(kb.columns) ? kb.columns.map(normalizeKanbanColumn) : [];
  columns = sortByOrder(columns, 'order');
  columns.forEach((col, i) => {
    col.order = i;
  });
  return {
    id: typeof kb.id === 'string' ? kb.id : uuid(),
    name: typeof kb.name === 'string' ? kb.name : 'Tablero',
    createdAt: typeof kb.createdAt === 'string' ? kb.createdAt : new Date().toISOString(),
    columns,
  };
}
 ...
 * @param {unknown} raw
 * @returns {{ ok: true, workspace: import('../core/types.js').Workspace } | { ok: false, error: string }}
 */
export function validateWorkspace(raw) {
  try {
    ...
    if (!('books' in o)) {
      return { ok: false, error: 'El archivo no contiene «books». ¿Es un export de Rinconcito narrativo?' };
    }
    ...
    if (ver > SCHEMA_VERSION) {
      return { ok: false, error: `Versión de esquema no soportada (${ver}). Actualiza Rinconcito narrativo.` };
    }
 ...
 * @param {import('../core/types.js').Workspace} ws
 * @returns {import('../core/types.js').Workspace}
 */
/**
 * @param {unknown} raw
